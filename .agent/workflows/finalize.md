...
当某个章节通过 `expert-review.md` 获得 **S级（优秀）** 评审后，使用本工作流进行最终定稿处理。
...
#### 情况1：小幅修改（推荐）
```
保留原 final.md，创建备份后直接修改
```
**操作流程**：
1. 备份当前定稿：`第X章_xxx_final.md` → `第X章_xxx_final_backup_YYYYMMDD.md`
2. 直接修改 `final.md`
3. 如需更新配图代码，添加新版本：`plot_fig_X_v2.m`
...
```
**操作流程**：
1. 在 `output/` 中修改至新版本（如 v4.md）
2. 通过 `expert-review.md` 获得S级评审
3. 重新执行完整定稿流程
4. 旧的 `final.md` 自动备份为 `final_backup_YYYYMMDD.md`

### 版本管理规则

```
final_output/
├── 第3章_3.3_xxx_final.md              # 当前最新定稿
├── 第3章_3.3_xxx_final_backup_20260111.md  # 备份（日期）
├── 第3章_3.3_xxx_final_backup_20260115.md  # 备份（日期）
└── figures/
    ├── 图3-4_时延演化轨迹_v1.png      # 原版本
    └── 图3-4_时延演化轨迹_v2.png      # 更新版本
```
...
绘图代码需参考 [thesis-code](../../thesis-code/) 中的仿真逻辑，确保：
...
2. 切换到代码目录：
   ```matlab
   cd('C:\Users\admin\Desktop\lunwen\wirting_workflow\final_output\figure_code');
   ```
3. 运行绘图脚本：
   ```matlab
...
### 路径格式
```markdown
![图3-4 时延演化轨迹对比](../../final_output/figures/图3-4_时延演化轨迹对比.png)
```
...
【修改内容】
- 修改位置：第5段
- 修改原因：导师建议补充碰撞频率的物理解释
- 修改内容：[具体修改描述]

【操作】
1. 自动备份为：第3章_3.3_xxx_final_backup_20260111.md
2. 在原文档中进行修改
3. 如需更新图表，生成新版本代码：plot_fig_X_v2.m
```
...
【章节信息】
- 章节编号：3.3
- 原定稿：final_output/第3章_3.3_xxx_final.md
- 新版本：output/第3章_3.3_xxx_v4.md（已通过S级评审）

【修改说明】
- 修改原因：根据审稿意见，增加了新的仿真对比分析
- 主要变化：新增图 3-7，调整了3.3.3节的推导逻辑

【操作】
1. 备份原定稿为：xxx_final_backup_20260111.md
2. 将 v4.md 复制为新的 final.md
3. 重新生成所有配图代码
4. 更新文档中的图片路径
```
