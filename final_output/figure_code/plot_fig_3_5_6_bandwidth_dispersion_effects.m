...
h1 = plot(B / 1e9, Delta_f_case1 / 1e6, 'o-', ...
    'Color', colors(1, :), 'LineWidth', 1.8, 'MarkerSize', 4);
hold on;
h2 = plot(B / 1e9, Delta_f_case2 / 1e6, 's-', ...
    'Color', colors(2, :), 'LineWidth', 1.8, 'MarkerSize', 4);
h3 = plot(B / 1e9, Delta_f_quad / 1e6, '--', ...
...
title('频轨展宽随带宽变化');
legend([h1 h2 h3], ...
    ['\fontname{Times New Roman}\Delta f_D - B ' ...
     '(\tau_1=-2 ps/Hz, \tau_2=1 fs^2/Hz)'], ...
    ['\fontname{Times New Roman}\Delta f_D - B ' ...
     '(\tau_1=1 ps/Hz, \tau_2=1 fs^2/Hz)'], ...
    ['\fontname{SimSun}二次近似 ' ...
     '\fontname{Times New Roman}(\propto B^2)'], ...
    'Interpreter', 'tex', ...
    'Location', 'northwest');
