...
进一步分析曲线的斜率可以揭示群时延色散率(GDD)的演化规律。根据式(3-18), $\frac{d\tau_g}{df}$ 与 $\left[1-(f_p/f)^2\right]^{-3/2}$ 成正比,这意味着曲线斜率在截止频率附近呈超线性增长。仿真数据表明,对于 $f_p = 29\ \text{GHz}$ 的情况,在高频区 $f = 40\ \text{GHz}$ 处,曲线斜率约为 $-0.02\ \text{ns/GHz}$;而在 $f = 30\ \text{GHz}$ 处,斜率已陡化至 $-0.8\ \text{ns/GHz}$,增大了40倍。
