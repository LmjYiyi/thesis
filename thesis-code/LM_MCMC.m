...
% 测量噪声估计 (用于似然函数)
sigma_meas = 0.1e-9; % 假设测量标准差为 0.1 ns
...
title('(a) n_e Trace Plot'); grid on;
...
sgtitle('Drude模型 MCMC 参数反演结果', 'FontSize', 14, 'FontWeight', 'bold');

% Figure 12: Corner Plot (2D 联合分布)
figure(12); clf;
...
sgtitle('Drude模型 Corner Plot: n_e vs \nu_e', 'FontSize', 14, 'FontWeight', 'bold');
...
title(sprintf('MCMC拟合结果 (n_e误差: %.2f%%)', (ne_mean - n_e)/n_e * 100)); 
